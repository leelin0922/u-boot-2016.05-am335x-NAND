 ...
 *
 */
#ifndef CONFIG_TARGET_AM335X_SBC_7109_455
...
int eeprom_i2c_parse_data(void)
{
...
#endif
		puts("I2C read failed in eeprom 0xfe()\n");
		AT24c02_eeprom.version=0x00;
		return 0;
	}

	AT24c02_eeprom.version=0+buffer[0]+(buffer[1]<<8);
	//printf("AT24c02_eeprom.version(0x%04x),buffer(%02x,%02x)\n",AT24c02_eeprom.version,buffer[0],buffer[1]);
	...
	//sprintf(buffer,"AT24c02_eeprom.version(0x%04x)\n",AT24c02_eeprom.version);
	//puts(buffer);
	...
	//puts("eeprom_set_resolution\n");
	eeprom_set_resolution();
...
int eeprom_i2c_synthesis_data(void)
{
	...
	//sprintf(tmp,"versione:%d (%d)\n",AT24c02_eeprom.version,AT24c02_eeprom.data.version);
	//puts(tmp);

	if(AT24c02_eeprom.version!=AT24c02_eeprom.data.version)
	{
		AT24c02_eeprom.version=AT24c02_eeprom.data.version;
		eepromversion[0]=0xff & AT24c02_eeprom.data.version;
		eepromversion[1]=0xff & (AT24c02_eeprom.data.version>>8);
#ifdef CONFIG_EEPROM_GPIO_I2C4
		//AT24c02_eeprom.write(0xfe, 1, eepromtmp, 2);
		i2c_gpio_write_eeprom(0xfe, 1, eepromversion, 2);
#else
		eeprom_i2c_write(0xfe, 1, eepromversion, 2);
		udelay( 11000 );
#endif
	}
...
#ifdef CONFIG_TARGET_AM335X_SBC_7109_455
	...
	if (mmc) 
	{
		...
		if (dev_desc!=NULL) 
		{
			if (fat_register_device((struct blk_desc *)dev_desc,1)==0) 
			{
				...
				sprintf(tmp,"read_version=0x%04x\n",read_version);
				puts(tmp);
				//if(read_version!=0x03)
				//{
				//	return -2;
				//}
				AT24c02_eeprom.data.version=read_version;
#ifndef CONFIG_TARGET_AM335X_SBC_7109_455
...
{
	if(AT24c02_eeprom.data.display[2]<0x0 || AT24c02_eeprom.data.display[2]>0x7)
		AT24c02_eeprom.data.display[2]=0x2;
	return AT24c02_eeprom.data.display[2];
}

void eeprom_set_resolution(void)
{
	switch(eeprom_i2c_get_EDID())
	{
		case 0x1:
			AT24c02_eeprom.width=640;
			AT24c02_eeprom.height=480;
			break;
		default:
		case 0x2:
			AT24c02_eeprom.width=800;
			AT24c02_eeprom.height=480;
			break;
		case 0x3:
			AT24c02_eeprom.width=800;
			AT24c02_eeprom.height=600;
			break;
		case 0x4:
			AT24c02_eeprom.width=1024;
			AT24c02_eeprom.height=600;
			break;
		case 0x5:
			AT24c02_eeprom.width=1024;
			AT24c02_eeprom.height=768;
			break;
		case 0x6:
			AT24c02_eeprom.width=1280;
			AT24c02_eeprom.height=800;
			break;
		case 0x7:
			AT24c02_eeprom.width=1366;
			AT24c02_eeprom.height=768;
			break;
		case 0x8:
			AT24c02_eeprom.width=1920;
			AT24c02_eeprom.height=1080;
			break;
	}
}

int eeprom_i2c_get_width(void)
{
	return AT24c02_eeprom.width;
}

int eeprom_i2c_get_height(void)
{
	return AT24c02_eeprom.height;
}

#define CMDLINE_ADD_QUIET
void set_7109_env(void)
{
	//optargs video=800x480@60
	char optprm[512]={0};
	...
	else
	{
		...
		{
			sprintf(optprm,"%d", AT24c02_eeprom.mbootdelay&0x0f);
			setenv("bootdelay",optprm);
		}
		else
			setenv("bootdelay","0");
	}
	...
	sprintf(optprm,"video=%dx%d@%u quiet %s ", AT24c02_eeprom.width, AT24c02_eeprom.height, AT24c02_eeprom.data.display[4],Backlightprm);
	//sprintf(optprm,"video=%dx%d-16@%u,if=RGB565,bpp=16 quiet %s ", AT24c02_eeprom.width, AT24c02_eeprom.height, AT24c02_eeprom.data.display[4],Backlightprm);
	//sprintf(optprm,"video=%dx%d-16@%u quiet %s ", AT24c02_eeprom.width, AT24c02_eeprom.height, AT24c02_eeprom.data.display[4],Backlightprm);
#else
	sprintf(optprm,"video=%dx%d@%u %s ", AT24c02_eeprom.width, AT24c02_eeprom.height, AT24c02_eeprom.data.display[4],Backlightprm);
	//sprintf(optprm,"video=%dx%d-16@%u,if=RGB565,bpp=16 %s ", AT24c02_eeprom.width, AT24c02_eeprom.height, AT24c02_eeprom.data.display[4],Backlightprm);
	//sprintf(optprm,"video=%dx%d-16@%u %s ", AT24c02_eeprom.width, AT24c02_eeprom.height, AT24c02_eeprom.data.display[4],Backlightprm);
#endif
	setenv("optargs",optprm);
}
